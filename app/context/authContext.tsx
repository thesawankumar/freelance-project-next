...
function useProtectedRoute(user: any) {
  ...
  React.useEffect(() => {
    ...
    const inAuthGroup = pathname ? pathname.includes("/login") : false;
    if (!user && !inAuthGroup) {
      router.push('/login');
...
function deleteTokenCookie() {
  ...
  const idToken = cookieList.filter((c) => c.match(/.idToken/g) && c.match(/CognitoIdentityServiceProvider/g))
  console.log(idToken);
  const date = new Date();
  // Set it expire in -1 days
  date.setTime(date.getTime() + (-1 * 24 * 60 * 60 * 1000));
  document.cookie = idToken+"=; expires="+date.toUTCString()+"; path=/";
}
